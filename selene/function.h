...
template<typename T>
struct is_primitive<Selene::function<T>> {
    static constexpr bool value = true;
};

template <typename R, typename...Args>
inline Selene::function<R(Args...)> _check_get(_id<Selene::function<R(Args...)>>,
                                            lua_State *l, const int index) {
    lua_pushvalue(l, index);
    return Selene::function<R(Args...)>{luaL_ref(l, LUA_REGISTRYINDEX), l};
}

template <typename R, typename... Args>
inline Selene::function<R(Args...)> _get(_id<Selene::function<R(Args...)>> id,
                                      lua_State *l, const int index) {
    return _check_get(id, l, index);
}

template <typename R, typename... Args>
inline void _push(lua_State *l, Selene::function<R(Args...)> fun) {
    fun.Push(l);
}
}
}
