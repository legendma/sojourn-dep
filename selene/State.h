...
public:
    ...
    bool LoadBinary( const std::wstring &file, const byte *data, size_t sz ) {
        using convert_type = std::codecvt_utf8<wchar_t>;
        std::wstring_convert<convert_type, wchar_t> converter;

        //use converter (.to_bytes: wstr->str, .from_bytes: str->wstr)
        std::string filename = converter.to_bytes( file );
        return LoadBinary( filename, data, sz );
    }

    bool LoadBinary(const std::string &file, const byte *data, size_t sz ) {
        ResetStackOnScopeExit savedStack(_l);

        int status = LUA_ERRERR;
        if( data )
        {
            status = luaL_loadbuffer( _l, (const char*)data, sz, lua_tostring(_l, -1) );
               /* lua_State *L, const char *buff, size_t size,
                const char *name*/
        }
        else
        {
            status = luaL_loadfile(_l, file.c_str());
        }
